...
def parse_user_and_amount(text):
    ...
    comment = ""
    if len(text.split()) > 2:
        p_comment = str.split(' ', 2 )
        comment = p_comment[2]

    return user,amount,comment

def save_to_db(user, owed_user, owed_amount, comment):
...
def lambda_handler(event, context):
    ...
    owed_user, owed_amount, comment = parse_user_and_amount(command_text)
